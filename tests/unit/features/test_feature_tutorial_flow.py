...
def test_feature_tutorial_dps_arrow_targets_value(page: Page):
    ...
    result = page.evaluate("""
        () => {
            ...
            const withinX = endX >= (rect.left - 36) && endX <= (rect.left - 6);
            const withinY = endY >= (rect.top + rect.height * 0.35) && endY <= (rect.bottom - 1);
...
def test_feature_tutorial_intern_arrow_avoids_cost_column(page: Page):
    ...
    result = page.evaluate("""
        () => {
            ...
            const inRightZone = endX >= (rect.right + 2) && endX <= (rect.right + 24);
            const inRowBand = endY >= (rect.top + rect.height * 0.18) && endY <= (rect.bottom - rect.height * 0.18);
