const Tutorial = (() => {
    ...
    function update() {
        ...
        const gameState = window.__SUMMAN_GAME_API__.getState();
        ...
        if (state.subStep === 0) {
            ...
            // PRIORITY 2: Core loop - Orb clicking
                ...
                const clicks = data;
                if (clicks < 5) message = window.Lang.t('tutorial_click_0_4');
                else if (clicks < 10) message = window.Lang.t('tutorial_click_5_9');
                else if (clicks < 15) message = window.Lang.t('tutorial_click_10_14');
                else message = window.Lang.t('tutorial_click_15_19');
            }
            // PRIORITY 3: Buy Intern
                ...
                if (internBtn && internBtn.offsetParent !== null) {
                    target = internBtn;
                    type = 'intern';
                    message = window.Lang.t('tutorial_exploit');
                }
            }
        }
        ...
        if (state.subStep > 0) {
            ...
            if (state.subStep === 1) {
                ...
                type = 'dps';
                message = window.Lang.t('tutorial_dps_expl');
        ...
        // Delegated Render
        window.TutorialOverlay.render(target, type, message, narrativeStep > 1 ? narrativeStep : 0);
        ...
        if (state.subStep >= 2) {
            window.TutorialOverlay.render(null, null, narrativeText, state.subStep); // Re-map subStep 2->2 etc.
        }
    }
    ...
    function handleNarrativeLogic() {
        ...
        if (state.subStep === 1) {
            ...
            res.step = 2;
            res.text = window.Lang.t('tutorial_mantra_1') || "1. CRECE";
            ...
            res.step = 3;
            res.text = window.Lang.t('tutorial_mantra_2') || "2. ACUMULA";
            ...
            res.step = 4;
            res.text = window.Lang.t('tutorial_mantra_3') || "3. UTILIZA";
    ...
    function complete() {
        ...
        state.active = false;
        if (window.TutorialOverlay) window.TutorialOverlay.clear();
    }
    ...
    function restart() {
        ...
        state.active = true;
        init();
        window.Utils.showToast(window.Lang.getLanguage() === 'en' ? 'Tutorial restarted!' : '¡Tutorial reiniciado!', 'info', 2000);
    }
